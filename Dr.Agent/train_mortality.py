...
if __name__ == '__main__':
    ...
    #val_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data_path, 'train'), listfile=os.path.join(args.data_path, 'val_listfile.csv'), period_length=48.0)
    ...
    train_raw = utils.load_data(train_reader, discretizer, None, args.small_part, return_names=True)
    ...
    for cur_name in os.listdir(demo_path):
        ...
        cur_episode = cur_episode.replace(".csv", "")  # Ensure ".csv" is removed correctly

        cur_file = os.path.join(demo_path, cur_name)  # More reliable file path joining
        cur_name_clean = os.path.splitext(cur_name)[0]  # Remove ".csv" extension properly
        ...
        idx_list.append(cur_name_clean)
    ...
    for each_chunk in range(args.epochs):
        ...
        for step, (batch_x, batch_y, batch_name) in enumerate(train_loader):
            ...
            for i in range(len(batch_name)):
                ...
                cur_demo = torch.tensor(demographic_data[idx_list.index(cur_name_clean)], dtype=torch.float32)
    ...
    test_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data_path, 'test'),
                                            ...
                                            period_length=48.0)
    test_raw = utils.load_data(test_reader, discretizer, None, args.small_part, return_names=True)
